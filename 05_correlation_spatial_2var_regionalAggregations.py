...
HiMAP regions and k-means clusters
"""
...
mnt = r'Z:/'
# regional aggregation / regional subsets (region_code = HiMAP)
...
def compute_and_write_regional(df_name, df_out_name_base, clim_or_morph, var_to_use, region, region_class):

    df_ = pd.read_csv(df_name, index_col=0)
    df_.index.name = 'RGI code'  # make nice and neat
    # regional subset
    regions_uq = np.unique(region.values)
    for region_i in regions_uq:
        ref_ss = ref_df.loc[ref_df[region_class] == region_i,region_class]
        ref_ss_ind = ref_ss.index
        df = df_.loc[ref_ss_ind]
        df_ind = df
        ...
        for t in a:
            comb0.append(t)
        if clim_or_morph == 'climateOnly':
            combs = [ci[0] + ';' + ci[1] for ci in comb0 if ci[0] != ci[1] and (
                        ci[0].split('_')[0] == ci[1].split('_')[0] or ci[0].split('_')[0] == 'MOD10' or
                        ci[1].split('_')[0] == 'MOD10')]
        else:
        ...
        out_cor_glac.to_csv(DIR_OUT + df_out_name_base + 'cor_2var_spatial_' + clim_or_morph +'_%s_%s' %(region_class, region_i)+ '.csv')
        out_slope_glac.to_csv(DIR_OUT + df_out_name_base + 'slope_2var_spatial_' + clim_or_morph +'_%s_%s' %(region_class, region_i)+ '.csv')
        out_pval_glac.to_csv(DIR_OUT + df_out_name_base + 'pval_2var_spatial_' + clim_or_morph +'_%s_%s' %(region_class, region_i)+ '.csv')

        out_cor_glac.to_pickle(DIR_OUT + df_out_name_base + 'cor_2var_spatial_' + clim_or_morph +'_%s_%s' %(region_class, region_i)+ '.pickle')
        out_slope_glac.to_pickle(DIR_OUT + df_out_name_base + 'slope_2var_spatial_' + clim_or_morph + '_%s_%s' %(region_class, region_i) + '.pickle')
        out_pval_glac.to_pickle(DIR_OUT + df_out_name_base + 'pval_2var_spatial_' + clim_or_morph +'_%s_%s' %(region_class, region_i)+ '.pickle')
        ...
        out_pval_glac1.to_csv(DIR_OUT + df_out_name_base + 'pval_2var(var1)_spatial_' + clim_or_morph +'_%s_%s' %(region_class, region_i)+ '.csv')
        out_pval_glac1.to_pickle(DIR_OUT + df_out_name_base + 'pval_2var(var1)_spatial_' + clim_or_morph +'_%s_%s' %(region_class, region_i)+ '.pickle')
        out_pval_glac2.to_csv(DIR_OUT + df_out_name_base + 'pval_2var(var2)_spatial_' + clim_or_morph +'_%s_%s' %(region_class, region_i)+ '.csv')
        out_pval_glac2.to_pickle(DIR_OUT + df_out_name_base + 'pval_2var(var2)_spatial_' + clim_or_morph +'_%s_%s' %(region_class, region_i)+ '.pickle')
        ...
        out_slope_glac1.to_csv(DIR_OUT + df_out_name_base + 'slope_2var(var1)_spatial_' + clim_or_morph +'_%s_%s' %(region_class, region_i) + '.csv')
        out_slope_glac1.to_pickle(DIR_OUT + df_out_name_base + 'slope_2var(var1)_spatial_' + clim_or_morph +'_%s_%s' %(region_class, region_i) + '.pickle')
        out_slope_glac2.to_csv(DIR_OUT + df_out_name_base + 'slope_2var(var2)_spatial_' + clim_or_morph +'_%s_%s' %(region_class, region_i) + '.csv')
        out_slope_glac2.to_pickle(DIR_OUT + df_out_name_base + 'slope_2var(var2)_spatial_' + clim_or_morph +'_%s_%s' %(region_class, region_i) + '.pickle')


region_cnt = 0
region_len = len(region_classifications)
for region_class_ in region_classifications:
    region_cnt += 1
    ...
    regions = ref_df[region_class_]

    compute_and_write_regional(df_name=df_name_, df_out_name_base=df_out_name_base_, clim_or_morph=clim_or_morph_,
                               var_to_use=var_to_use_, region=regions, region_class=region_class_)
